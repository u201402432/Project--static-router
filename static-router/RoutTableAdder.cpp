...
void CRoutTableAdder::OnBnClickedOk() // 라우팅테이블 추가버튼 클릭시
{
	...
	if(m_flag_u.GetCheck())
		flag = flag | 0x01; // Up flag set
	if(m_flag_g.GetCheck())
		flag = flag | 0x02; // Gateway flag set
	if(m_flag_h.GetCheck())
		flag = flag | 0x04; // Host-specific flag set

	CDialog::OnOK();
}


void CRoutTableAdder::OnBnClickedCancel() // 라우팅테이블 취소버튼 클릭시
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	OnCancel();
}
