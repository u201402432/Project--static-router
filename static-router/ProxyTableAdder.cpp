...
void ProxyTableAdder::OnBnClickedOk() // 추가 버튼 클릭 시
{
	...
	sscanf(Buffer1,"%.02X:%.02X:%.02X:%.02X:%.02X:%.02X", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5], sizeof(Buffer1));
...
void ProxyTableAdder::OnBnClickedCancel() // 취소버튼 클릭 시
{
	// TODO: Add your control notification handler code here
	OnCancel();
}
